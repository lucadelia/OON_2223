...
class Node:
    def __init__(self, in_dict):
        ...
        def propagate (self, signal_information):   # node.propagate(signal_information) begin the "propagate" for the node
            actual_path = signal_information.path   # saves the actual path of the signal_inf in the variable "actual_path"
            if len(actual_path)>1:                  # condition to break the recursivity -> we want at least 2 elements
                line_name = actual_path[:2]         # line_name contains the line path (composed by 2 elements: AB for ex.)
                line = self.successive[line_name]   # the successive node is set
                signal_information.update_path()    # the signal_information object update his path (method of sign_inf class)
