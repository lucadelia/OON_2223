...
class Line:
    ...
    def nli_generation(self, signal_information):
        # Evaluate the total amount of NLI generated by the non-linear interface noise
        n_span = self.n_amplifiers+1
        n_nli = (16/(27*pi))*np.log((pi*pi*self.beta2*Rs*Rs*N_channel**(2*Rs/freq))/(2*alpha_lin))*(self.gamma*self.gamma*alpha_lin*l_eff*l_eff)/(self.beta2*Rs**3)
        NLI = signal_information.signal_power ** 3 * n_nli * n_span * Bn    # Non-linear interference
        return NLI
